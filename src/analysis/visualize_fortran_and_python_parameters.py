...
def plot_results_comparison(df, colnames, path, title):
    ...
    # plot settings
    colors = ['royalblue', 'firebrick', 'seagreen', 'orange', 'black',
              'darkslateblue', 'r', 'khaki']
    ...
    ax.set_yticks(0.5 * nr_cols + np.array(range(
        0, y_max, (nr_cols + 1))))
    ...
    plt.title(title, fontsize=28, y=1 + 2.5 / nr_rows, x=-0.6, loc='left',
              weight='bold')
...
if __name__ == '__main__':
    ...
    estimated_df = pd.read_csv(estimated_path.format(model, dataset),
                               index_col='index')
    ...
    plot_results_comparison(
        df=df, colnames=['Python', 'Fortran'],
        path=ppj('OUT_ANALYSIS', '{}_{}/comparison_plot'.format(
            model, dataset)),
        title=('Comparison of Python and Fortran results\nBars show '
